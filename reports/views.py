...
# context. Each report loops over tenants explicitly and does its work inside
# tenant_context, which scopes every .objects query below to that tenant.
# Before this, the branch loop and the recipient list both spanned the whole
# database — one tenant's revenue figures would have been emailed to another
# tenant's recipients as soon as a second tenant existed.
...
def generate_daily_report():
    ...
    for tenant in Tenant.objects.all():
        with tenant_context(tenant):
            recipients = list(
                ReportRecipient.objects.filter(is_active=True).values_list('email', flat=True)
            )
            if not recipients:
                continue  # لا يوجد مستلمون نشطون لهذا المستأجر
